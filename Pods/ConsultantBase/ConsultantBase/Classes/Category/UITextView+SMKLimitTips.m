...
- (void)setPlaceHolderLabel:(NSString *)placeholder {
    ...
    CGRect rect = [placeholder boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-10, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:13.]} context:nil];
    self.placeholderLabel.frame = CGRectMake(10, 0, rect.size.width, rect.size.height);
...
- (void)setWordcountLable:(NSNumber *)limitLength {
     ...
     *  字数限制
     */
    self.wordCountLabel = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-70, CGRectGetHeight(self.frame) - 20, 60, 20)];
